...
parameters {
  ...
  vector[P*K] theta_vec[N];
  #cov_matrix[P*K] theta_vec_cov_matrix;
  cholesky_factor_cov[P*K] theta_vec_cov_matrix;
}
...
model {
  ...
  theta_vec[1]          ~  multi_normal_cholesky(rep_vector(0, K*P) , theta_vec_cov_matrix);
  theta_vec[2:N_before] ~  multi_normal_cholesky(theta_vec[1:(N_before-1)] , theta_vec_cov_matrix);
...
generated quantities {
  ...
  theta_vec_pred[(N_before+1):N] = multi_normal_cholesky_rng(rep_array(theta_vec_pred[N_before], N_after), theta_vec_cov_matrix);
